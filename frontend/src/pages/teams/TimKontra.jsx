...
export default function TimKontra() {
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      await api.post("/kontra", { ...form, medsos: form.medsos.filter(Boolean) });
  ...
  async function del(id) { if (!confirm("Hapus?")) return; await api.delete(`/kontra/${id}`); load(); }
  ...
  return (
    <div data-testid="kontra-page">
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Form Profiling" color="#EF4444" testid="kontra-form-card">
          <form onSubmit={submit} className="space-y-4" data-testid="kontra-form">
            ...
            <Button type="submit" disabled={busy} data-testid="kontra-submit" className="w-full bg-amber-500 hover:bg-amber-400 text-zinc-950 btn-tactical rounded-sm h-10">
              {busy ? "Menyimpan..." : "Simpan Profiling"}
            </Button>
          </form>
        </Card>
        ...
        <Card title="Daftar Profiling" testid="kontra-list-card">
          {items.length === 0 ? <Empty /> : (
            <ul className="space-y-3">
              {items.map((it) => (
                <li key={it.id} className="bg-zinc-950 border border-zinc-800 rounded-sm p-3" data-testid={`kontra-item-${it.id}`}>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button onClick={() => del(it.id)} data-testid={`kontra-delete-${it.id}`} className="text-zinc-500 hover:text-red-400"><Trash size={14} weight="bold" /></button>
                  </div>
                </li>
              ))}
            </ul>

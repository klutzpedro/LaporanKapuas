...
- **PDF Layout Cleanup — Orphan Headers, Map Whitespace, Label Overlap (2026-05)**: (1) **Orphan header fix**: Tambah helper `begin_section(title, count, first_item_h)` yang force new page bila ruang tidak cukup untuk header + minimal 1 content card. Diterapkan ke section LID/KONTRA/GAL/PIKET. Tidak ada lagi "judul yatim" di akhir halaman. (2) **Map whitespace fix**: Di `_draw_geoint_fullpage`, render PNG dengan aspect ratio yang MATCH frame inner aspect (compute `target_aspect = frame_w_mm / frame_h_mm`, set `map_height_px = 1800 / target_aspect`). Map sekarang mengisi penuh bordered frame tanpa padding kosong. (3) **Label overlap fix**: Algoritma greedy de-overlap di `render_papua_map` — sort markers by Y, untuk setiap label cek bbox terhadap labels yang sudah placed, kalau overlap push down LINE_H=22px sampai clear. Tambah connector lines (faint dark) dari marker ke label kalau di-offset. Font diturunkan 22→16pt biar lebih banyak label muat tanpa clutter. Visual-verified via rasterized page (pdftoppm + Gemini analyzer, 100% confidence).

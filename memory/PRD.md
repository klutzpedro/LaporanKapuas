...
- **Numbered Map Markers + Tile Cache for Concurrency (2026-05)**: (1) **Numbered markers**: Label di peta GEOINT diganti dari nama target ke NOMOR (1,2,3,4,5...) yang match dengan kolom NO di tabel di atasnya. Tiap nomor di-render WHITE TEXT di atas COLORED DISC: merah untuk AKTIF, hijau untuk NON-AKTIF. (2) **Update banner di peta**: Strip gelap translucent di atas peta menampilkan "POSISI OPM · UPDATE TANGGAL DD/MM/YYYY · JAM HH.MM WIB" dari `max(updated_at)` di antara geoint items, dikonversi ke WIB. (3) **Concurrency optimization (CRITICAL for 20+ users)**: PDF gen dari 57s → 1-3s. Implementasi: (a) module-level cache `_PAPUA_BASE_CACHE` untuk base Papua map image (tiles fetched ONCE from OSM, reused thousands of times); thread-safe dengan `_PAPUA_BASE_LOCK`; (b) `build_summary_pdf` di-`run_in_threadpool` agar tidak block event loop; (c) `generated_reports` pakai `replace_one(upsert=True)` atomik (tidak ada race delete+insert); (d) Mongo indexes di startup untuk semua `report_date` columns + composite `(report_date, updated_at)` di 6 collections; (e) `users.email` unique index, `generated_reports.report_date` + `ai_summaries.report_date` unique; (f) uvicorn workers 2→4 + `--limit-concurrency 200 --timeout-keep-alive 75` di deploy.sh. Load-tested: 26 concurrent ops (21 browse + 5 PDF gen) selesai dalam 10s, 0 failures.

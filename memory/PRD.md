...
- **Tim GAL Statistik per Platform (2026-05)**: Tim GAL sekarang punya form khusus untuk input jumlah penggalangan per kategori (NARASI/VIDEO/MEME) × platform (Instagram/Facebook/Twitter-X/TikTok/YouTube) = 15 angka per hari. Backend: collection baru `gal_stats_reports` (unique index per `report_date`), endpoint `POST/GET /api/gal/stats` dengan upsert. Frontend: grid 3×5 di-bawah list konten dengan total per row/col + grand total + stacked bar chart preview (1 bar per platform, 3 segmen warna). PDF: section GAL diawali dengan stacked bar chart "STATISTIK PENGGALANGAN PER PLATFORM" + total post + numbers "N X · V Y · M Z" per row + legend. Verified visual via Gemini analyzer (100% confidence).
...
- **PDF Layout Polish (2026-02)**: (1) **Orphan headers fixed** — `begin_section(title, count, first_item_h)` sekarang reserve ruang min `7mm header + first_item_h` sebelum draw, otomatis `new_page()` kalau item tidak muat. Diterapkan ke LID, KONTRA, GAL, PIKET. (2) **GEOINT dedicated full page** — `_draw_geoint_fullpage()` dipanggil setelah `new_page()` sehingga selalu mulai di halaman bersih. Peta Papua di-render dengan `target_aspect = frame_w/frame_h` sehingga raster image match frame → tidak ada whitespace inside frame border. (3) **Map labels de-overlap** — label di peta diganti dari NAMA TARGET (yang sebelumnya tumpang tindih) menjadi NOMOR (1..N) centered di marker. Nama lengkap tetap muncul di kolom NAMA TARGET pada tabel kompak di atas peta. Visual-verified via Gemini PDF analyzer (4 halaman, semua section header attached ke konten, peta fill 100%, label tidak overlap).
